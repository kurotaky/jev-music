...
def render(path: str, f: features.Features, result: dict | None, elapsed: dict) -> str:
    ...
    lines.append(f"  テンポ     {f.bpm} BPM（ビート {f.beat_count} 個 / 規則性 {f.beat_regularity}）")
    ...
    lines.append(f"  音色       重心 {f.brightness_hz:.0f} Hz / 打楽器比 {f.percussive_ratio} / 毎秒 {f.onset_rate} 音")
    if result:
        a = result["answers"]
        lines += ["", "[Jev] 判定"]
